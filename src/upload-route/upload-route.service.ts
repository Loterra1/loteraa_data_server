...
export class UploadRouteService {
  ...
  async getAllUserSmartContracts(userID: string) {
    const record = await this.smartContractRepo.find({ where: { userID } });
    return { message: 'All Smart contracts retrieved successfully', success: true, data: record };
  }
  ...
  async getAllUserUploadededData(userID: string) {
    const record = await this.dataFilesRepo.find({ where: { userID } });
    return { message: 'All User Data Files retrieved successfully', success: true, data: record };
  }
